...
if (typeof module !== 'undefined' && module.exports) {
    ...
    drawMoon() {
        ...
        this.moon.x = this.canvas.width - 150 + Math.sin(time * 0.5) * 30;
        ...
        this.ctx.beginPath();
        this.ctx.arc(this.moon.x + 20, this.moon.y, this.moon.radius + 5, 0, Math.PI * 2);
        this.ctx.fillStyle = 'rgba(8, 8, 24, 0.8)';
        this.ctx.fill();

        // Craters
        this.moon.craters.forEach(crater => {
            this.ctx.beginPath();
            this.ctx.arc(this.moon.x + crater.x, this.moon.y + crater.y, crater.r, 0, Math.PI * 2);
    ...
    updateShootingStars() {
        ...
        this.shootingStars.forEach(star => {
            ...
            const opacity = 1 - star.progress;

            this.ctx.beginPath();
            this.ctx.moveTo(x, y);
            this.ctx.lineTo(x - star.length * (1 - star.progress), y - star.length * (1 - star.progress));
    ...
    createShootingStar() {
        const startX = Math.random() * this.canvas.width * 0.8 + this.canvas.width * 0.2;
